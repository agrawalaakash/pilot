...
class WarehouseExplore(Node):
    ...
    def reach_shelves(self, shelf_index, map_info, map_array, img):
        ...
        self.qr_random = self.qr_code_str
        self.logger.info(f"self.qr_random: {self.qr_random}")
...
#due to this counter value will update in the next itteration to find index of next shelf
    ...
    def shelf_objects_callback(self, message):
        ...
        #     if 7 > count > 2:
        #         sequence[int(self.qr_code_str.split("_")[0])]= sequence_publisher(message.object_name, message.object_count, message.qr_decoded)
        #         self.logger.info(f"length of sequence: {len(sequence)}")
            
        # elif int(self.qr_code_str.split("_")[0]) == self.obj_iter:
        #     if 7 > count > 2:
        ...
        #         if self.once:
        #             self.obj_iter += 1
        #         self.logger.info(f"obj_iterrate: {self.obj_iter}")
        ...
        #             # self.logger.info(f"lengt of current_item: {current_item}")
        ...
        #             if self.once:
        #                 self.obj_iter += 1
        #                 self.logger.info(f"obj_iterrate inside while: {self.obj_iter}")
        #             # If the goal is to check subsequent keys, simpler incrementing is usually better.
        #         self.once = False


        try:
            current_qr_no = int(self.qr_code_str.split("_")[0])
        except ValueError:
            self.logger.warn(f"Invalid QR code '{self.qr_code_str}', skipping message.")
            return
        ...
        # Case 1: This is a shelf we have ALREADY PROCESSED. Ignore it.
        # (This check solves your bug)
        if current_qr_no < self.obj_iter:
            self.logger.info(f"Already processed shelf {current_qr_no}, ignoring.")
            return

        # Case 2: This is the EXACT shelf we are waiting for.
        elif current_qr_no == self.obj_iter:
            self.logger.info(f"Processing expected shelf {current_qr_no}.")
            
            # Publish this shelf's data
            shelf_data_message = WarehouseShelf()
            shelf_data_message.object_name = message.object_name
            shelf_data_message.object_count = message.object_count
            shelf_data_message.qr_decoded = self.qr_code_str
            self.publisher_shelf_data.publish(shelf_data_message)
            # self.publisher_shelf_data.publish(shelf_data_message) # Publish twice for reliability

            # Increment iterator ONLY ONCE per shelf
            if self.once:
                self.obj_iter += 1
                self.logger.info(f"obj_iter incremented to: {self.obj_iter}")
                self.once = False # Mark as "incremented" so this block doesn't run again

            # Now, process any sequential shelves stored in the dictionary
            while sequence.get(self.obj_iter) != None:
                self.logger.info(f"Processing sequenced shelf {self.obj_iter} from dictionary.")
                
                # Pop the item, removing it from the sequence
                current_item = sequence.pop(self.obj_iter) 
                ...
                self.obj_iter += 1 # Keep incrementing as we process
                self.logger.info(f"obj_iter incremented to: {self.obj_iter}")

        # Case 3: This is a FUTURE shelf. Store it in the sequence.
        elif current_qr_no > self.obj_iter:
            # Only add it to the dictionary if it's not already there
            if current_qr_no not in sequence:
                self.logger.info(f"Storing future shelf {current_qr_no} in sequence.")
                sequence[current_qr_no] = sequence_publisher(
                    message.object_name, 
                ...
                self.logger.info(f"Shelf {current_qr_no} is already in sequence, ignoring.")
